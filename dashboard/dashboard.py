...
df = pd.read_csv(
    "https://raw.githubusercontent.com/deantama/dicoding-bike-sharing/main/dashboard/clean_bike_share_hour.csv"
)
...
def create_seasonly_users_df(df):
    ...
    seasonly_users_df["season"] = pd.Categorical(
        seasonly_users_df["season"], categories=["Spring", "Summer", "Fall", "Winter"]
    )
...
with st.sidebar:
    # menambahkan logo
    st.image(
        "https://raw.githubusercontent.com/deantama/dicoding-bike-sharing/main/img/img_logo.png"
    )
...
fig2 = px.bar(
    ...
    barmode="group",
    color_discrete_sequence=["skyblue", "orange", "red"],
    title="Count of bikeshare rides by weekday",
).update_layout(xaxis_title="", yaxis_title="Total Rides")
...
st.caption("Copyright (c), created by Dean Pratama Putra")
